    ...
    function show_opt_ac() {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                ac_powland_opt_keys = Object.keys(ac_powland_opt);
                for (var i1 = 0; i1 < ac_powland_opt_keys.length; i1++) {
                    key = ac_powland_opt_keys[i1];
                    col=ac_powland_opt[key];
                    //console.log(col);
                    checked = false;
    ...
    function check_state_ac(type, elem) {
        if (ac_opt_powland[type] == undefined)
            ac_opt_powland[type] = {};
        ac_opt_powland[type]["checked"] = elem.checked;
        console.log(ac_opt_powland)
    }
    
    function check_state_ac_energy(type, elem) {
        if (ac_powland_opt[type] == undefined)
            ac_powland_opt[type] = {};
        ac_powland_opt[type]["checked"] = elem.checked;
        console.log(ac_powland_opt)
    }
    ...
    function draw_graph_ac(data_array, interval, compare) {
        ...
        if (interval == true) {
            fdate = $("#fdate").val()
            ldate = $("#ldate").val()
            url_ac = "/home_station/powland_data?fdate=" + fdate + "&ldate=" + ldate + "&interval=true&compare=false"+ac_opt_str;
        } else {
            if (compare == true) {
                date1 = $("#date1").val()
                date2 = $("#date2").val()
                url_ac = "/home_station/powland_data?date1=" + date1 + "&date2=" + date2 + "&interval=false&compare=true"+ac_opt_str;
            } else {
                items = $("#items_interval").val()
                    //console.log(items)
                url_ac = "/home_station/powland_data?items=" + items + "&interval=false&compare=false"+ac_opt_str;
            }
        }
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                ac_opt_powland_keys = Object.keys(ac_opt_powland);
                ac_powland_opt_keys = Object.keys(ac_powland_opt);
                var graph_ind = 0;
                data_array_powland = [];
                showkeys_graph_position = {}
                for (var i = 0; i < ac_opt_powland_keys.length; i++) {
                    if (ac_opt_powland[ac_opt_powland_keys[i]].checked == true)
                        showKeys.push(ac_opt_powland_keys[i])
                }
                
                for (var i = 0; i < ac_powland_opt_keys.length; i++) {
                    if (ac_powland_opt[ac_powland_opt_keys[i]].checked == true)
                        showKeys_energy.push(ac_powland_opt_keys[i])
                } 
                console.log(showKeys);
                for (var i = 0; i < showKeys.length; i++) {
                    data_array_powland[i] = {
                        type: "line",
                        axisYType: "secondary",
                        name: showKeys[i],
                        showInLegend: true,
                ...
                console.log(data_array_powland);
                console.log(showKeys);
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    for (var j = 0; j < Object.keys(item).length; j++) {
                        ...
                        for(showKeys_energy_val of showKeys_energy){
                            if (showKeys_energy != undefined && itemrow.includes(showKeys_energy_val)) {
                                disp=true;
                                break;
                            }
                        }
                        ...
                        if(disp){
                            ...
                            data_array_powland[showkeys_graph_position[itemrow]].dataPoints.push({
                ...
                chart = new CanvasJS.Chart("graph", {
                    ...
                    data: eval(data_array_powland) //eval(data_array)
                });
                //console.log(data_array_powland);
                //chart["data"] = 
                console.log(chart);
